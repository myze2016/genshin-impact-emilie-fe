...
export const addCharacterWeapon = async (payload) => {
  try {
    const response = await api.post(`/character-weapon`, payload)
...
export const removeCharacterWeapon = async (payload) => {
  try {
    const response = await api.post(`/character-weapon/delete-by-weapon`, payload)
     if (response?.data?.success) {
      toast.success(response?.data?.message, { transition: Slide, hideProgressBar: true, autoClose: 2000 })
    } else {
      toast.error(response?.data?.message, { transition: Slide, hideProgressBar: true, autoClose: 2000 })
