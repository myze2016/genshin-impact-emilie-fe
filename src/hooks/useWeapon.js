...
export const getWeaponSearch = (characterId, weaponTypeId, refetch, search, page, rowsPerPage) => {
  const [data, setData] = useState([])
  const [loading, setLoading] = useState(true)
  const [total, setTotal] = useState(true)
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      if (!weaponTypeId && !characterId) {
          setLoading(false)
          return
      }
      try {
       const response = await api.get(`/weapon-search?weapon_type_id=${weaponTypeId}&character_id=${characterId}&search=${search}&page=${page}&rows_per_page=${rowsPerPage}`);
    ...
    fetchData()
  }, [characterId, weaponTypeId, refetch, search, page, rowsPerPage])
